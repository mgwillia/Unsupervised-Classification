...
def main():
    ...
    train_dataset = get_train_dataset(p, train_transformations, 
                                        split='train+unlabeled', to_neighbors_strangers_dataset = True)
    ...
    torch.save(gt_targets, os.path.join(p['scanf_dir'], 'gt_targets.pth.tar'))
    torch.save(cluster_predictions, os.path.join(p['scanf_dir'], 'cluster_predictions.pth.tar'))
