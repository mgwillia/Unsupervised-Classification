...
from utils.train_utils import scanh_train

FLAGS = argparse.ArgumentParser(description='SCANH Loss')
...
def main():
    ...
    if args.mode == 'train':
        # Checkpoint
        if os.path.exists(p['scanh_checkpoint']):
            print(colored('Restart from checkpoint {}'.format(p['scanh_checkpoint']), 'blue'))
            checkpoint = torch.load(p['scanh_checkpoint'], map_location='cpu')
            ...
            print(colored('No checkpoint file at {}'.format(p['scanh_checkpoint']), 'blue'))
            start_epoch = 0
            best_loss = 1e4
            best_loss_head = None
        ...
        for epoch in range(start_epoch, p['epochs']):
            ...
            # Train
            print('Train ...')
            scanh_train(train_dataloader, model, criterion, optimizer, epoch, p['update_cluster_head_only'])
            ...
            if (epoch + 1) % 5 == 0:
                ...
                scanh_stats = scan_evaluate(predictions)
                print(scanh_stats)
                lowest_loss_head = scanh_stats['lowest_loss_head']
                lowest_loss = scanh_stats['lowest_loss']
                ...
                if lowest_loss < best_loss:
                    ...
                    torch.save({'model': model.module.state_dict(), 'head': best_loss_head}, p['scanh_model'])
    ...
    model_checkpoint = torch.load(p['scanh_model'], map_location='cpu')
    ...
    clustering_stats = hungarian_evaluate(model_checkpoint['head'], predictions, 
                            ...
                            confusion_matrix_file=os.path.join(p['scanh_dir'], 'confusion_matrix.png'))
    print(clustering_stats)
