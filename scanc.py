...
from utils.train_utils import scanc_train

FLAGS = argparse.ArgumentParser(description='SCANC Loss')
...
def main():
    ...
    if args.mode == 'train':
        # Checkpoint
        if os.path.exists(p['scanc_checkpoint']):
            print(colored('Restart from checkpoint {}'.format(p['scanc_checkpoint']), 'blue'))
            checkpoint = torch.load(p['scanc_checkpoint'], map_location='cpu')
            ...
            print(colored('No checkpoint file at {}'.format(p['scanc_checkpoint']), 'blue'))
            start_epoch = 0
            best_loss = 1e4
            best_loss_head = None
    ...
    torch.save(gt_targets, os.path.join(p['scanc_dir'], 'gt_targets.pth.tar'))
    torch.save(cluster_predictions, os.path.join(p['scanc_dir'], 'cluster_predictions.pth.tar'))
    clustering_stats = hungarian_evaluate(model_checkpoint['head'], predictions, 
                            class_names=val_dataset.dataset.classes, 
                            compute_confusion_matrix=True, 
                            confusion_matrix_file=os.path.join(p['scanc_dir'], 'confusion_matrix.png'))
    print(clustering_stats)
