...
from utils.train_utils import scankl_train
...
def main():
    ...
    if p['update_cluster_head_only']:
        print(colored('WARNING: SCANKL will only update the cluster head', 'red'))
    ...
    if args.mode == 'train':
        # Checkpoint
        if os.path.exists(p['scankl_checkpoint']):
            print(colored('Restart from checkpoint {}'.format(p['scankl_checkpoint']), 'blue'))
            checkpoint = torch.load(p['scankl_checkpoint'], map_location='cpu')
            ...
            print(colored('No checkpoint file at {}'.format(p['scankl_checkpoint']), 'blue'))
            start_epoch = 0
            best_loss = 1e4
            best_loss_head = None
        ...
        for epoch in range(start_epoch, p['epochs']):
            ...
            # Train
            print('Train ...')
            scankl_train(train_dataloader, model, criterion, optimizer, epoch, p['update_cluster_head_only'])
            ...
            if (epoch +  1) % 5 == 0:
                ...
                print('Evaluate based on SCANKL loss ...')
                scankl_stats = scan_evaluate(predictions)
                print(scankl_stats)
                lowest_loss_head = scankl_stats['lowest_loss_head']
                lowest_loss = scankl_stats['lowest_loss']
                ...
                if lowest_loss < best_loss:
                    ...
                    torch.save({'model': model.module.state_dict(), 'head': best_loss_head}, p['scankl_model'])
    ...
    print(colored('Evaluate best model based on SCANKL metric at the end', 'blue'))
    ...
    clustering_stats = hungarian_evaluate(model_checkpoint['head'], predictions, 
                            ...
                            confusion_matrix_file=os.path.join(p['scankl_dir'], 'confusion_matrix.png'))
    print(clustering_stats)
